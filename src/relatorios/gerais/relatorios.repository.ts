// Acesso a dados do módulo relatorios
...
import type { Relatorios, CriarRelatoriosInput, AtualizarRelatoriosInput } from "./relatorios.types";
...
export class RelatoriosRepository {
  ...
  async buscarPorId(id: string): Promise<Relatorios | null> {
    const db = getLocalDb();
    // TODO: query real
    return null;
  }

  async criar(dados: CriarRelatoriosInput): Promise<Relatorios> {
    const db = getLocalDb();
    // TODO: insert real + marcar para sincronização
    throw new Error("Não implementado");
  }

  async atualizar(id: string, dados: AtualizarRelatoriosInput): Promise<Relatorios> {
    const db = getLocalDb();

// Acesso a dados do módulo usuarios
...
import type { Usuarios, CriarUsuariosInput, AtualizarUsuariosInput } from "./usuarios.types";
...
export class UsuariosRepository {
  ...
  async criar(dados: CriarUsuariosInput): Promise<Usuarios> {
    const db = getLocalDb();
    // TODO: insert real + marcar para sincronização
    throw new Error("Não implementado");
  }

  async atualizar(id: string, dados: AtualizarUsuariosInput): Promise<Usuarios> {
    const db = getLocalDb();

// Controller HTTP do módulo vendas
import type { Request, Response, NextFunction } from "express";
import { VendasService } from "./vendas.service";
import { criarVendasSchema, atualizarVendasSchema } from "./vendas.schema";
...
export const vendasController = {
  ...
  async criar(req: Request, res: Response, next: NextFunction) {
    try {
      const dados = criarVendasSchema.parse(req.body);
  ...
  async atualizar(req: Request, res: Response, next: NextFunction) {
    try {
      const dados = atualizarVendasSchema.parse(req.body);

// Acesso a dados do módulo notas
...
import type { Notas, CriarNotasInput, AtualizarNotasInput } from "./notas.types";
...
export class NotasRepository {
  ...
  async buscarPorId(id: string): Promise<Notas | null> {
    const db = getLocalDb();
    // TODO: query real
    return null;
  }

  async criar(dados: CriarNotasInput): Promise<Notas> {
    const db = getLocalDb();
    // TODO: insert real + marcar para sincronização
    throw new Error("Não implementado");
  }

  async atualizar(id: string, dados: AtualizarNotasInput): Promise<Notas> {
    const db = getLocalDb();

...
import type { Dre, CriarDreInput, AtualizarDreInput } from "./dre.types";
...
export class DreRepository {
  ...
  async buscarPorId(id: string): Promise<Dre | null> {
    const db = getLocalDb();
    // TODO: query real
    return null;
  }

  async criar(dados: CriarDreInput): Promise<Dre> {
    const db = getLocalDb();
    // TODO: insert real + marcar para sincronização
    throw new Error("Não implementado");
  }

  async atualizar(id: string, dados: AtualizarDreInput): Promise<Dre> {
    const db = getLocalDb();
